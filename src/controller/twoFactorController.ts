...
export class TwoFactorController {
  ...
  async validate(req: Request, res: Response): Promise<void> {
    try {
      ...
      // ValidateOtpService lança Error puro (sem statusCode) para código
      // inválido/expirado/não solicitado — sempre foi 401, preserva o status.
      if (error instanceof Error) {
        res.status(401).json({ error: error.message });
        return;
      }
      handleControllerError(res, error, "Erro ao validar código 2FA");
    }
  }
